...
@component
def TreeViewPanel() -> ui.Row:
    def handle_tree_select(node_key:str) -> None:
        selected_tree_node.set(node_key)
        current_directory.set(node_key.replace(f"{os.path.sep}.", os.path.sep))

    def handle_tree_expand(node_key:str, is_expanded:bool) -> None:
    ...
    dir_parts = [
        f"{x}{os.path.sep}" for x in cur_dir.rstrip(os.path.sep).split(os.path.sep)
    ]
    dir_tree = {}
    leaf = dir_tree
    expanded = []
    ...
    leaf_contents = [
        x for x in leaf_contents if x[0] != "."
    ]  # Get rid of invisible directories
    ...
    return ui.Row(
        ...
        children=[
            ui.Column(
                props={"spacing": 5},
                children=[
                    ui.Label("Directory Explorer", props={"class": "h1"}),
                    ui.TreeView(
                        data=dir_tree,
